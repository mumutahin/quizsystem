...
public class DeleteQuizSetFrame extends JFrame {
    public DeleteQuizSetFrame(String email) {
        setTitle("Delete Quiz Set");
        setSize(400, 300);
        ...
        centerPanel.add(new JLabel("Select Quiz to Delete:"), BorderLayout.NORTH);
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "Delete this quiz?");
        if (confirm == JOptionPane.YES_OPTION) {
            QuizDao.deleteQuiz(quizzes.get(selectedIndex - 1).getId()); // note: -1
            JOptionPane.showMessageDialog(this, "Quiz deleted.");
            dispose();
            new DeleteQuizSetFrame(email); // reload fresh list
        }
        });
